...
public class ImageCraft {
    ...
    public static BufferedImage getResizedImageByName(String filename, boolean fullsize) {
        ...
        else if ((tmp.getWidth() < 128) && ((tmp.getHeight() < 128)))
            img.getGraphics().drawImage(tmp, (img.getWidth() / 2) - (tmp.getWidth() / 2), (img.getHeight() / 2) - (tmp.getHeight() / 2), null);
    ...
    public static List<BufferedImage> getImageByName(Player p, String filename, int x, int y) {
        ...
        int mx = x;
        int my = y;
        BufferedImage image = getImageByName(p, filename, false);
        ...
        for (int j = 0; j < my; j++)
            for (int i = 0; i < mx; i++) {
                BufferedImage tmp = new BufferedImage(128, 128, BufferedImage.TYPE_INT_ARGB);
                tmp.getGraphics().drawImage(bigImage.getSubimage(i * 128, j * 128, 128, 128), 0, 0, null);
                list.add(tmp);
            }
        return list;
    }
    ...
    public static File getFileWithoutExtension(String directory, String fileName) {
        ...
        f = new File(directory + File.separator + fileName + ".jpg");
        if (f.exists()) return f;
        f = new File(directory + File.separator + fileName + ".jpeg");
        if (f.exists()) return f;
        f = new File(directory + File.separator + fileName + ".gif");
        if (f.exists()) return f;
        return null;
    }
